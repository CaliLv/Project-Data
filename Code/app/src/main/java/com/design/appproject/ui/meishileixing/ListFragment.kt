...
import com.design.appproject.ui.meishileixing.ListFilterDialog
...
import com.design.appproject.databinding.MeishileixingcommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.meishileixing.ListModel
import com.design.appproject.ui.meishileixing.ListAdapter
 ...
 * 美食类型列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_MEISHILEIXING)
class ListFragment : BaseBindingFragment<MeishileixingcommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("FoodType","meishileixing")/*查询索引*/

    override fun initEvent() {
        setBarTitle("Food Type",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("meishileixing","Add"))|| Utils.isAuthFront("meishileixing","Add")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_MEISHILEIXING).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_MEISHILEIXING)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("Prompt","Are you sure you want to delete it?") {
                      HomeRepository.delete<Any>("meishileixing",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_MEISHILEIXING,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun MeishileixingcommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("meishileixing","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
        ...
        mIsBack.yes {
                mListModel.page("meishileixing", requestParams)
        }.otherwise {
                mListModel.list("meishileixing", requestParams)
        }
    }
}