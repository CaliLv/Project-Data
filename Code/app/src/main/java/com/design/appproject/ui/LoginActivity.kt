...
class LoginActivity : BaseBindingActivity<ActivityLoginLayoutBinding>() {
    ...
    private fun ActivityLoginLayoutBinding.verify(): Boolean {
        return when {
            ...
            passwordEt.text.toString().isNullOrEmpty()-> {
                "Please enter your password".showToast()
                false
            }
            usertypeBs.text.toString().trim() == "Please select the type of user to log in to" -> {
                "Please select the type of user to log in to".showToast()
                false
            }
            else -> true
        }
    ...
    val roleNameList by lazy {
        roleMenusList?.filter { it.hasFrontLogin == "Yes" }?.map { it.roleName } ?: listOf("")
    }

    private fun ActivityLoginLayoutBinding.initUsertypeTv() {
        usertypeMfl.isVisible = roleNameList.size>1
        usertypeBs.setOptions(roleNameList, "Please select the login user type", listener = object : BottomSpinner.OnItemSelectedListener {
    ...
    private fun loginSuccess(token:String){
        ...
        UserRepository.session<UserInfoBean>().observeKt {
            it.getOrNull()?.let {
                ...
                StorageUtil.encode(CommonBean.VIP_KEY, it.data.vip == "Yes")
