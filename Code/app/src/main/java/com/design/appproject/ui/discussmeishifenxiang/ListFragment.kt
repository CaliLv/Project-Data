...
import com.design.appproject.ui.discussmeishifenxiang.ListFilterDialog
...
import com.design.appproject.databinding.DiscussmeishifenxiangcommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.discussmeishifenxiang.ListModel
import com.design.appproject.ui.discussmeishifenxiang.ListAdapter
 ...
 * 美食分享评论表列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_DISCUSSMEISHIFENXIANG)
class ListFragment : BaseBindingFragment<DiscussmeishifenxiangcommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("Food sharing comment form",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("discussmeishifenxiang","新增"))|| Utils.isAuthFront("discussmeishifenxiang","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DISCUSSMEISHIFENXIANG).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("Prompt","Are you sure you want to delete it") {
                      HomeRepository.delete<Any>("discussmeishifenxiang",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_DISCUSSMEISHIFENXIANG,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun DiscussmeishifenxiangcommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("discussmeishifenxiang", requestParams)
        }.otherwise {
                mListModel.list("discussmeishifenxiang", requestParams)
        }
    }
}