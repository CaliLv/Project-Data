...
import com.design.appproject.ui.yongcanjihua.ListFilterDialog
...
import com.design.appproject.databinding.YongcanjihuacommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.yongcanjihua.ListModel
import com.design.appproject.ui.yongcanjihua.ListAdapter
 ...
 * 用餐计划列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_YONGCANJIHUA)
class ListFragment : BaseBindingFragment<YongcanjihuacommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("Meal plan",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("yongcanjihua","Add"))|| Utils.isAuthFront("yongcanjihua","Add")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_YONGCANJIHUA).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_YONGCANJIHUA)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("Prompt","Are you sure you want to delete it?") {
                      HomeRepository.delete<Any>("yongcanjihua",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_YONGCANJIHUA,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun YongcanjihuacommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("yongcanjihua", requestParams)
        }.otherwise {
                mListModel.list("yongcanjihua", requestParams)
        }
    }
}