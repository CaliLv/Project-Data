...
class MainActivity : BaseBindingActivity<ActivityMainLayoutBinding>() {
    ...
    private val fragments = mutableListOf(
        ...
        ARouter.getInstance().build(CommonArouteApi.PATH_FRAGMENT_LIST_MEISHIFENXIANG).withBoolean("mHasBack",false).navigation() as Fragment,
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            if (System.currentTimeMillis() - exitTime > 2000) {
                Toast.makeText(this, "Press again to exit${AppUtils.getAppName()}", Toast.LENGTH_SHORT).show()
